...
with st.expander("📖 Documentation", expanded=False):
    ...
    **Parameters:**
     ...
     `x` (str | None): Xaxis column (default: index)
     `y` (str | list[str] | None): Yaxis column(s)
...
with st.container(border=True):
    with st.defer_mount(
        "charts_plotly",
        placeholder_height=380,
        prefetch_types=["plotly_chart"],
    ) as mount:
        if mount.active:
            try:
                import plotly.graph_objects as go
...
with st.container(border=True):
    with st.defer_mount(
        ...
        prefetch_types=["vega_lite_chart"],
    ) as mount:
        if mount.active:
            try:
                import altair as alt
...
with st.expander("📖 Documentation", expanded=False):
    ...
    **Parameters:**
     ...
     `spec` (dict): VegaLite specification
     `use_container_width` (bool): Fill container width
     `theme` (str | None): `"streamlit"` or `None`
     `key` (str | None): Stable widget key
    """)
    ...
    with st.defer_mount(
        ...
        if mount.active:
            ...
            vl_spec = {
                ...
                "height": 300,
                "title": "VegaLite Scatter (no extra dependencies)",
            }
...
with st.expander("📖 Documentation", expanded=False):
    ...
    **Parameters:**
    ...
    Rendered serverside as PNG — no client dependency on Matplotlib.
    """)
...
with st.expander("📖 Documentation", expanded=False):
    ...
    **Parameters:**
    ...
    Bokeh renders clientside via BokehJS — interactive zoom/pan included.
    """)
...
with st.expander("📖 Documentation", expanded=False):
    ...
    **Parameters:**
    ...
    Rendered via `@hpccjs/wasm` — no serverside Graphviz install needed.
    """)
